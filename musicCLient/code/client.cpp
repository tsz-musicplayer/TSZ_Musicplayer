...
const char *ser_ip = "192.168.1.7";
unsigned ser_port = 24;
...
void Client::connetct(const char *ip_address, unsigned port)
{
...
{
    socket_.connect(TCP::endpoint(asio::ip::address_v4::from_string(ser_ip), ser_port));
...
void Client::getSonglist(std::string songlist)
{
    message("songlist");
    save_status = 2;
    socket_.send(asio::buffer(songlist, download_path_max_size));
